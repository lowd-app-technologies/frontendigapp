...
const InstagramCloseFriends = ({ className }: InstagramCloseFriendsProps) => {
  ...
  const handleHealthCheck = async () => {
    try {
      const response = await fetch(
        "https://pythonfastapi-production-437e.up.railway.app/health"
      );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setLoading(true);
    setMessages(["Verificando servidor..."]);
    setShowForm(false);
  
    try {
      // Chama o handleHealthCheck e aguarda 5 segundos
      await handleHealthCheck();
      setMessages((prev) => [...prev, "Servidor ativo. Continuando..."]);
      ...
      ws.onmessage = (event) => {
        ...
        if (event.data.includes("Digite o código de dois fatores")) {
          ...
          setMessages((prev) => [...prev, "Aguardando código de dois fatores..."]);
        }
      };
    } catch (error) {
      ...
      setMessages((prev) => [
        ...prev,
        "Erro ao verificar o servidor. Tente novamente mais tarde.",
      ]);
      setLoading(false);
      setShowForm(true);
    }
  ...
  const handleStopService = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setMessages((prev) => [
          ...prev,
          `Erro ao interromper o processo: ${response.status} ${response.statusText}`,
        ]);
      }
    } catch (error) {
      ...
      setMessages((prev) => [
        ...prev,
        `Erro ao interromper o processo: ${
  ...
  return (
    <div
      className={`flex flex-col items-center p-6 bg-neutral justify-center ${
        className || ""
      }`}
    ...
    >
      <Card
        ...
        footer={{
          content: (
            <span className="text-xs text-gray-500 flex items-center text-green-500">
              <HiCheckCircle className="mr-1 text-lg" /> Seus dados são tratados
              com segurança
            </span>
          ),
        }}
